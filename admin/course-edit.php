    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("UPDATE courses SET
                title=?, category=?, duration=?, fee=?,
                short_description=?, description=?, requirements=?, outcomes=?,
                intake_dates=?, image_url=?, is_active=?, is_featured=?, sort_order=?
                WHERE id=?");
            $stmt->execute([
...
// Use POST data on error, else DB data
$d = empty($errors) ? $course : array_merge($course, $_POST);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Edit Course - RETI Admin</title>
...
<div class="main-wrap">
    <header class="top-header">
        ...
        <div class="header-title">
            <h1>Edit Course</h1>
            <p><a href="courses.php" style="color:var(--primary);">Courses</a> / <?= htmlspecialchars($course['title']) ?></p>
        </div>
    ...
    <main class="page-content">
        ...
        <?php if ($success): ?>
        <div class="alert alert-success"><i class="fas fa-check-circle"></i> Course updated successfully.</div>
        <?php endif; ?>
        <?php if (!empty($errors)): ?>
            ...
            <div class="form-layout">
                        ...
                        <div style="padding:1.5rem;">
                            <div class="form-grid" style="gap:1.25rem;">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" class="form-control" required value="<?= htmlspecialchars($d['title']) ?>">
                                </div>
                                <div class="form-grid form-grid-2">
                                    <div class="form-group">
                                        ...
                                        <select name="category" class="form-control" required>
                                            ...
                                            <option value="<?= $val ?>" <?= $d['category']===$val?'selected':'' ?>><?= $lbl ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Duration</label>
                                        <input type="text" name="duration" class="form-control" value="<?= htmlspecialchars($d['duration'] ?? '') ?>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Short Description</label>
                                    <textarea name="short_description" class="form-control" rows="2"><?= htmlspecialchars($d['short_description'] ?? '') ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label>Full Description</label>
                                    <textarea name="description" class="form-control" rows="5"><?= htmlspecialchars($d['description'] ?? '') ?></textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="outcomes" class="form-control" rows="3"><?= htmlspecialchars($d['outcomes'] ?? '') ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label>Intake Dates</label>
                                    <input type="text" name="intake_dates" class="form-control" value="<?= htmlspecialchars($d['intake_dates'] ?? '') ?>" placeholder="e.g., January, April, July, October">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="form-sidebar">
                    <div class="section-card" style="margin-bottom:1.25rem;">
                        ...
                        <div style="padding:1.25rem;display:flex;flex-direction:column;gap:1rem;">
                            <div class="form-group">
                                ...
                                <input type="number" name="fee" class="form-control" step="0.01" min="0" value="<?= htmlspecialchars($d['fee'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                ...
                                <img id="imgPreview" class="img-preview" src="../<?= htmlspecialchars($d['image_url']) ?>" alt="Preview">
                                <?php else: ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="sort_order" class="form-control" min="0" value="<?= (int)($d['sort_order'] ?? 0) ?>">
                    ...
                    <div style="display:flex;flex-direction:column;gap:.75rem;">
                        <button type="submit" class="btn btn-primary" style="width:100%;"><i class="fas fa-save"></i> Update Course</button>
                        <a href="courses.php" class="btn btn-light" style="width:100%;text-align:center;"><i class="fas fa-times"></i> Cancel</a>
                    </div>
                </div>
            </div>
        </form>
